#define DE_Y KC_Z
#define DE_AE KC_QUOT
#define DE_UE KC_LBRC
#define DE_OE KC_SCLN
// #define IT_HASH ALGR(KC_QUOT)
#define DE_SS KC_MINS
#define DE_Z KC_Y
// #define IT_AT ALGR(KC_SCLN)
#define DE_AT ALGR(KC_Q)
#define DE_ACUT KC_EQL
// #define IT_DQUO S(KC_2)
// #define IT_EXLM S(KC_1)
#define DE_HASH KC_NUHS
#define DE_PIPE ALGR(KC_NUBS)
// #define IT_LBRC ALGR(KC_LBRC)
// #define IT_LPRN S(KC_8)
// #define IT_LCBR S(ALGR(KC_LBRC))
#define DE_AMPR S(KC_6)
#define DE_DLR S(KC_4)
#define DE_CIRC KC_GRV
#define DE_EURO ALGR(KC_E)
#define DE_PERC S(KC_5)
#define DE_TILD ALGR(KC_RBRC)
#define DE_PLUS KC_RBRC
#define DE_LESS KC_NUBS
#define DE_MINS KC_SLSH
// #define IT_EQL S(KC_0)
// #define IT_SLSH S(KC_7)
#define DE_LCBR ALGR(KC_7)
#define DE_RCBR ALGR(KC_0)
#define IT_RABK S(KC_NUBS)
#define IT_LABK KC_NUBS

#define DE_EQL S(KC_0)
#define DE_LBRC ALGR(KC_8)
#define DE_RBRC ALGR(KC_9)
#define DE_LPRN S(KC_8)
#define DE_RPRN S(KC_9)
